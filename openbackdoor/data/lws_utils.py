...
def wrap_util(dataset: dict, batch_size, target_label, tokenizer, poison_rate):
    ...
    for key in dataset.keys():
        dataloader[key] = DataLoader(prepare_dataset_parallel(dataset[key], poison_rate, train=(key == 'train')),
                                     batch_size=batch_size, shuffle=True, num_workers=5)
        # get_dataloader(dataset[key], batch_size=batch_size)
    return dataloader
